...
print("Found", len([f for f in os.listdir(save_loc) if f.endswith(".pt") and f.startswith("image")]), "files to segment.")
...
with torch.no_grad():
	for f in os.listdir(save_loc):
		...
		nm = this_input_nm.split(os.sep)[-1].replace("image", "class")
